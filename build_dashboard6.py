...
let trainingGroupsMode = 'relative'; // 'relative' (scaled to bodyweight) or 'absolute' (raw lb)
try { trainingGroupsMode = localStorage.getItem('jaxst_mbb_training_groups_mode') || 'relative'; } catch(e){}
function setTrainingGroupsMode(mode){
  trainingGroupsMode = mode;
  try { localStorage.setItem('jaxst_mbb_training_groups_mode', mode); } catch(e){}
  renderTrainingGroups();
}
function renderTrainingGroups(){
  const container = document.getElementById('groupsContent');
  if (!container) return;
  const mode = trainingGroupsMode;
  ...
  svg += `<text x="${w/2}" y="${h-6}" fill="var(--muted)" font-size="9.5" text-anchor="middle">${strengthAxisLabel}</text>`;
  svg += `<text x="12" y="${h/2}" fill="var(--muted)" font-size="9.5" text-anchor="middle" transform="rotate(-90 12 ${h/2})">Power (z-score)</text>`;
  results.forEach(r=>{
    ...
    svg += `<text x="${x.toFixed(1)}" y="${(y-8).toFixed(1)}" fill="var(--text)" font-size="9" text-anchor="middle">${r.name.split(',')[0]}</text>`;
